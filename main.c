...
// Funzione che restituisce il punto di interesse con un determinato ID
...
// Funzione che restituisce l'indice di un punto di interesse all'interno di una
// città
...
// Modify create_POI_internal to return the created POI
...
int create_POI(city _city, char *POI_name) {
  if (_city->n_POI < _city->max_POI) {
    ...
    _city->n_POI++;
    return _city->n_POI - 1; // Restituisci l'indice effettivo del nuovo punto di interesse
  } else {
    return -1;
  }
}
...
// Funzione che verifica se una strada esiste tra due punti di interesse
// all'interno di una città
...
void create_road_internal(city c, POI *POI_src, POI *POI_dest,
                          char *_m_transport, int *times) {
  ...
  (*POI_src)->n_roads++;
  (*POI_src)->roads = (struct _road *)realloc(
      (*POI_src)->roads, sizeof(struct _road) * (*POI_src)->n_roads);
  ...
  (*POI_dest)->n_roads++;
  (*POI_dest)->roads = (struct _road *)realloc(
      (*POI_dest)->roads, sizeof(struct _road) * (*POI_dest)->n_roads);
...
// Funzione che verifica se un punto di interesse esiste all'interno di una
// città
...
// Funzione per creare una nuova strada tra due punti di interesse all'interno
// di una città
int create_road(city _city, POI *POI_src, POI *POI_dest, char *m_transport,
                int *times, bool one_way) {
...
int main() {
  // Crea città
  city myCity = create_city("Campobasso");

  // Crea POI
  int POI1_ID = create_POI(myCity, "POI 1");
  int POI2_ID = create_POI(myCity, "POI 2");

  // getPOI from city and id
  POI POI1 = get_POI(myCity, POI1_ID);
  POI POI2 = get_POI(myCity, POI2_ID);
  ...
  char m_transport[M_TRANSPORT] = {'a', 'A', 'b'};
  int times[M_TRANSPORT] = {10, 20, 30};
  bool one_way = false;
  int result = create_road(myCity, &POI1, &POI2, m_transport, times, one_way);


  if (result == -1) {
    printf("Strada non creata!\n");
  } else {
    printf("Strada creata con successo!\n");

    // Controlli se esiste una strada tra i due POI
    bool roadExists = road_exists(myCity, POI1, POI2);
    if (roadExists) {
      printf("C'e' una strada tra i due POI!\n");
    } else {
      printf("Non c'e' nessuna strada tra i due POI.\n");
    }
  }

  // Libera la memoria
  for (int i = 0; i < myCity->n_POI; ++i) {
    free(myCity->POI[i].name);
    free(myCity->POI[i].roads);
  }
  free(myCity->POI);
  for (int i = 0; i < MAX_SIZE; ++i) {
    free(myCity->roads[i]);
